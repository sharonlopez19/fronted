...
import { Vacante, VacanteService } from '../../../services/gestion.service';
...
export class GestionComponent implements OnInit {
  ...
  guardarVacante(): void {
    ...
    // Aquí asumimos nomVacante es obligatorio (ajusta si otros campos de la imagen lo son)
    if (!this.vacanteSeleccionada.nomVacante) { // Validando solo nomVacante
      Swal.fire('Error', 'El nombre de la vacante es obligatorio.', 'error'); // Mensaje de validación
      return;
    }
    // Si 'codigo' sigue siendo un campo de formulario pero no de la API,
    // o si quieres validar más campos de la imagen, AJUSTA la validación AQUÍ.
    // Ejemplo: if (!this.vacanteSeleccionada.nomVacante || !this.vacanteSeleccionada.salario) { ... }
    ...
    request$.pipe(
        ...
        next: (responseVacante) => {
            ...
            if (this.vacanteSeleccionada.idVacantes === undefined || this.vacanteSeleccionada.idVacantes === null) {
                 ...
                 this.vacanteSeleccionada = {
                     idVacantes: undefined, nomVacante: '', descripVacante: '', salario: 0, expMinima: '', cargoVacante: '', catVacId: undefined
